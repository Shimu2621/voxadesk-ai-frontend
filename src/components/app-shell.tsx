...
import { Bot, CalendarDays, ChartNoAxesCombined, Inbox, LayoutDashboard, Library, PhoneCall, Settings } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white md:grid md:grid-cols-[250px_1fr]">
      <aside className="border-b border-white/10 bg-slate-950/95 p-5 md:min-h-screen md:border-b-0 md:border-r">
        <Link href="/" className="text-xl font-black tracking-tight"><span className="text-cyan-400">Voxa</span>Desk AI</Link>
        ...
        <nav className="mt-8 grid grid-cols-2 gap-1 md:grid-cols-1">
          {navigation.map(([label, href, Icon]) => (
            <Link key={href} href={href} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-300 hover:bg-white/10 hover:text-white">
              <Icon size={17} /> {label}
            </Link>
          ))}
        </nav>
